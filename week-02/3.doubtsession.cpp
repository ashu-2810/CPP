#include<iostream>
using namespace std;
int main(int argc, char const *argv[])
{
    // how to link two files
    /*  
        -> all files must be in same folder to execute it.
        -> make 1.cpp file create a function
        -> make 2.cpp file create only till function not to add main function
        -> make a 2.h file(header file) write the code to define the location of 2.cpp function
        -> include 2.h (header file) on both file(1.cpp, 2.cpp)
        -> in terminal run g++ -c 1.cpp to create 1.o (object file of 1.cpp) simillarly for 2.cpp
        -> after creating 1.o, 2.o in terminal run g++ 1.o 2.o -o run to make run.exe
        -> run the run.exe file by ./run.exe
    */


    return 0;
}